...
class Governor(BarangayAgent):
    ROLE = "governor"
    HOME_ZONE = "resA"
    WORK_ZONE = "capitol"
    TRAIT_BIAS = {"ambition": 0.80, "competence": 0.65, "family_loyalty": 0.70}

    SCHEDULE = [
        (6,  8,  "resA",   "sleeping"),
        (8,  12, "capitol","morning_sessions"),
        (12, 13, "market", "public_presence"),
        (13, 17, "capitol","afternoon_meetings"),
        (17, 19, "resA",   "family_time"),
        (19, 22, "capitol","evening_lobbying"),
        (22, 24, "resA",   "sleeping"),
    ]

    def _starting_wealth(self): return random.gauss(3_000_000, 500_000)
    def _base_income(self):     return random.gauss(8_000, 500)

    def _init_goals(self):
        pool = [
            Goal("win_reelection",      "Win gubernatorial re-election",       priority=0.95),
            Goal("dynasty_consolidate", "Consolidate family's political grip",  priority=0.80, public=False),
            Goal("province_develop",    "Deliver provincial infrastructure",    priority=0.75),
            Goal("enrich_family",       "Grow family's business empire",        priority=0.65, public=False),
            Goal("control_contracts",   "Control provincial contract awards",   priority=0.70, public=False),
            Goal("suppress_opposition", "Neutralize political rivals",          priority=0.55, public=False),
        ]
        self.goals = random.sample(pool, k=4)
    ...
    def _decide_next(self):
        ...
        corrupt_roll = (self.traits.greed - self.traits.integrity +
                        gf.dynasty_score * 0.4)
        loc = self.get_scheduled_location()
        if corrupt_roll > 0.25 and random.random() < corrupt_roll * 0.35:
            declared = random.gauss(5_000_000, 500_000)
            actual   = declared * random.uniform(0.50, 0.75)
            self._execute_action({
                "action_id": "control_contracts", "location": "capitol",
                "duration_hours": 3, "corruption": True,
                "declared_budget": declared, "actual_budget": actual,
            })
        else:
            self.start_task(loc, "governing", duration_ticks=random.randint(3, 5))


class ViceGovernor(BarangayAgent):
    ROLE = "vice_governor"
    HOME_ZONE = "resA"
    WORK_ZONE = "capitol"
    TRAIT_BIAS = {"integrity": 0.58, "ambition": 0.70}

    def _starting_wealth(self): return random.gauss(2_000_000, 400_000)
    def _base_income(self):     return random.gauss(6_000, 400)

    def _init_goals(self):
        pool = [
            Goal("preside_board",    "Preside Sangguniang Panlalawigan", priority=0.85),
            Goal("run_for_governor", "Position for gubernatorial run",   priority=0.75),
            Goal("cover_for_gov",    "Shield governor from scrutiny",    priority=0.50, public=False),
        ]
        self.goals = random.sample(pool, k=2)

    def get_scheduled_location(self) -> str:
        h = self.model.clock.hour
        if 8 <= h < 17: return "capitol"
        return self.HOME_ZONE
    ...
    def _decide_next(self):
        if self.pending_action:
            ...
            return
        gf = self.model.global_factors
        gov = next((a for a in self.model._agents_list
                    if isinstance(a, Governor)), None)
        if (gov and self.dynasty_enabled and
                gov.family_id and gov.family_id == self.family_id):
            gf.audit_strength = max(0.0, gf.audit_strength - 0.002)
        else:
            gf.audit_strength = min(1.0, gf.audit_strength + 0.001)
        self.start_task("capitol", "presiding_board", duration_ticks=3)


class ProvincialBoardMember(BarangayAgent):
    ROLE = "board_member"
    HOME_ZONE = "resA"
    WORK_ZONE = "capitol"
    TRAIT_BIAS = {"ambition": 0.60}

    def _starting_wealth(self): return random.gauss(800_000, 150_000)
    def _base_income(self):     return random.gauss(3_500, 300)

    def _init_goals(self):
        pool = [
            Goal("pass_ordinance",  "Pass provincial ordinance",          priority=0.80),
            Goal("align_governor",  "Support governor's agenda",          priority=0.65),
            Goal("expose_gov",      "Expose governor's corruption",       priority=0.55),
            Goal("secure_kickback", "Secure contractor kickback",         priority=0.45, public=False),
        ]
        self.goals = random.sample(pool, k=2)
    ...
    def _decide_next(self):
        if self.pending_action:
            ...
            self.pending_action = None
            return
        self.start_task("capitol", "board_work", duration_ticks=2)
...
ROLE_CLASS_MAP = {
    ...
    "board_member":     ProvincialBoardMember,
    "mayor":            Mayor,
...
# 1000-agent population for Mabuhay Province (2020-2026)
# Provincial officials: 1+1+10 = 12
# Municipal officials:  3+3+24+8 = 38
# Business:             8+80+25 = 113
# Civic workers:        60+20+15+4+3 = 102
# Citizens:             205+150+200+80+100 = 735
# TOTAL: 1000
ROLE_DISTRIBUTION = {
    "governor":         1,
    "vice_governor":    1,
